...
import { useSession, type SessionScan } from "@/contexts/session-context"
...
export function LawImpactDetector({ onRequestRevision }: LawImpactDetectorProps) {
  const { regulations, scans: scanHistory, addScan, deleteScan } = useSession();
  ...
  const saveScanResult = (output: DetectLawImpactOutput, scannedLawText: string, scannedLawName?: string) => {
    addScan({
  ...
  const handleScan = async (textToScan?: string, scanLawName?: string) => {
    ...
    if (!targetText.trim()) {
      toast({ variant: "destructive", title: "법령 미입력", description: "분석할 법령 텍스트를 입력해주세요." });
      return;
    }
    if (regulations.length === 0) {
      toast({ variant: "destructive", title: "규정 없음", description: "라이브러리에 규정을 먼저 등록해주세요." });
      return;
    }
    ...
    try {
      const output = await detectLawImpact({
        ...
        regulations: regulations.map(r => ({ id: r.id, fileName: r.fileName, content: r.content }))
      });
      setResult(output);
      ...
      setResult(null);
      toast({ variant: "destructive", title: "스캔 실패", description: error.message || "법령 영향 분석 중 오류가 발생했습니다." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAutoScan = async () => {
    ...
    try {
      ...
      setResult(null);
      toast({ variant: "destructive", title: "스캔 실패", description: error.message || "AI 법령 스캔 중 오류가 발생했습니다." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteScan = (scanId: string, e: React.MouseEvent) => {
    ...
    toast({ title: "삭제 완료", description: "스캔 이력이 삭제되었습니다." });
  };
          ...
          <Card className="shadow-sm border-slate-200">
            <CardHeader className="pb-3 border-b bg-slate-50/50">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Button variant="outline" size="sm" onClick={handleSampleScan} disabled={loading} className="h-8 text-xs font-bold">
                    <Sparkles className="w-3 h-3 mr-1" />
                    샘플 법령으로 스캔
                  </Button>
                  <Button variant="default" size="sm" onClick={handleAutoScan} disabled={loading} className="h-8 text-xs font-bold bg-emerald-600 hover:bg-emerald-700">
          ...
          {sortedHistory.length > 0 ? (
            <div className="space-y-3">
              {sortedHistory.map((scan) => (
                <Card
                  key={scan.id}
                  className={`shadow-sm border-slate-200 cursor-pointer transition-all hover:shadow-md ${selectedScan?.id === scan.id ? 'ring-2 ring-primary' : ''}`}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <span className="text-sm font-medium text-slate-700">{formatDate(scan.scannedAt)}</span>
                        </div>
        ...
        {loading && (
          <div className="py-20 flex flex-col items-center justify-center space-y-4 bg-white rounded-3xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500 font-medium animate-pulse">
              {regulations.length}개의 사내 규정을 법령과 대조 분석하고 있습니다...
            </p>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {selectedScan && (
              <div className="flex items-center space-x-2 text-sm text-slate-500">
                ...
                {selectedScan.lawName && <><span>|</span><span>{selectedScan.lawName}</span></>}
              </div>
            )}
