...
import { Book, UploadCloud, Trash2, FileText, Search, Loader2, Plus, FileUp, AlertCircle } from "lucide-react"
...
export function KnowledgeBase() {
  const { regulations, addRegulation, deleteRegulations } = useSession();
  ...
  const handleManualUpload = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "저장 실패",
        description: "규정 저장 중 오류가 발생했습니다.",
      });
    } finally {
  ...
  const confirmDelete = () => {
    ...
    deleteRegulations(deleteTarget.ids);
    // 삭제된 항목을 선택 목록에서도 제거
    ...
    toast({
      title: "삭제 완료",
      description: `${deleteTarget.ids.length}개의 규정이 라이브러리에서 제거되었습니다.`,
    });
  };
