 ...
 * law.go.kr 판례 검색 도구가 통합되어 관련 판례를 인용합니다.
 */

import { ai } from '@/ai/genkit';
import { z } from 'genkit';
import { searchPrecedentsTool } from '@/ai/tools/law-go-kr-tool';
...
const AnswerComplianceQuestionOutputSchema = z.object({
  ...
  precedentCitations: z.array(z.object({
    caseNumber: z.string().describe('사건번호'),
    caseName: z.string().describe('사건명'),
    summary: z.string().describe('판결 요지'),
    relevance: z.string().describe('질문과의 관련성'),
  })).optional().describe('관련 판례 인용 목록'),
});
...
- 반드시 한국어로 답변
- 질문과 관련된 법령이나 판례가 있다면 searchPrecedents tool을 사용하여 검색하고 답변에 인용할 것
- 검색된 판례는 'precedentCitations' 필드에 포함시키되, 각 항목에 caseNumber(사건번호), caseName(사건명), summary(판결 요지), relevance(질문과의 관련성)를 한국어로 기재할 것
